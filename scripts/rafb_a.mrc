/rafb {
  ...
  while %switches != $null {
    if $1 == $null {
      echo 2 * /rafb: insufficient parameters
      halt
    }
    ...
    if %s == u {
      if $regex($1,/[^0-9A-Fa-f]/) {
        echo 2 * /rafb: invalid user key $1
        halt
      }
    ...
    elseif %s == p {
      if !$istok(0 1 2,$1,32) {
        echo 2 * /rafb: invalid privacy boolean $1
        halt
      }
    ...
    elseif %s == x {
      if !$istok(N 10M 1H 1D 1M,$1,32) {
        echo 2 * /rafb: invalid expiration $1
        halt
      }
      var %expiration $+(api_paste_expire_date=,$upper($1),&)
    }
    else {
      echo 2 * /rafb: invalid switch - $+ %s
      halt
    }
    tokenize 32 $2-
  }
  ...
  if $regex($1,/[^0-9A-Fa-f]/) {
    echo 2 * /rafb: invalid dev key $1
    halt
  }
  var %devkey $+(api_dev_key=,$1,&)
  tokenize 32 $2-
  if !$regex(rafb,$1-,/^("[^"]+")/) {
    echo 2 * /rafb: malformed file path
    halt
  }
  if !$exists($regml(rafb,1)) {
    echo 2 * /rafb: no such file $regml(rafb,1)
    halt
  }
  ...
  while (%sockname == $null) || ($sock(%sockname) != $null) {
    var %sockname $+(rafb.,$rand(1,999999999999))
  }
  sockopen %sockname rafb.me 80
  sockmark %sockname $+(key=1&text=) $1-
}
/rafb.return {
  ;echo -s . $1-
  .timer $+ $1 off
